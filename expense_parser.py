...
def add_expense(date_made, estab_name, amount,expense_array,categories_dic):
	expense_input = Expense(date_made, estab_name, amount, categories_dic.get(str(estab_name),"unlisted_category"))
...
def parse_xls_html(path, row_input,expense_array,categories_dic):
	...
	while keep_parsing:
		...
		add_expense(date_made, set_name(establishment), amount,expense_array,categories_dic)
		row +=1
	try:
